...
#			$applets_priority_hash{$1}=$priority_index;
		...
		# form is %setting=number
			...
			my $k=$1;
			($settings{$1})=($_=~/=(.*)/); # is this right? is $1 even set here?
...
sub setAppletsToDisplay () {
	my $MAX_WIDTH=getTerminalWidth()-1;
	my $display_width=0;
	my $applet_counter=0;
	...
	unless ($display_width < $MAX_WIDTH) { # if we're 
		eraseLTRLists();
...
# DSCF = display string color formatted

sub getDateCF {
	my $s = `date +$settings{"date_format"}`;
	chomp ($s);
	if (length($s) > $APPLETS_MAX_WIDTHS{"date"}) {
...
sub getTimeNF {
	my $s = `date +$settings{"time_format"}`;
	chomp ($s);
	if (length($s) > $APPLETS_MAX_WIDTHS{"date"}) {
...
#		print ("$vscf\n");
		...
		$s*=100;
		$s=int($s);
		if (length("vol $s\%") > $APPLETS_MAX_WIDTHS{"volume"}) {
			die ("Error: volume string too long.\n");
		}
		$s=$COLORS{"LABEL"}."vol ".$COLORS{"VOLUME"}.$s.$COLORS{"UNITS"}."\%";
		return ($s);
	}
	else {
...
sub getDisplayStrings() { #mostly for debugging at this point
	setAppletsToDisplay();

	debugShit();
	
	my $display_string_CF="";
	my $display_string_NF="";
	my $stopper = 50;
	
	for (my $i=0; $i<@display_divided_LTR_array and $i<$stopper; $i++) {
		print ("$display_divided_LTR_array[$i]\n");
		if ($display_divided_LTR_array[$i] eq "date") {
			$display_string_CF .= getDateCF();
#			$display_string_NF .= getDateNF();
			unless ($display_divided_LTR_array[$i+1] eq "tail" 
				or $display_divided_LTR_array[$i+1] eq ""
					or !exists($display_divided_LTR_array[$i+1])) {
				$display_string_CF .= $COLORS{"DIVIDER"};
				$display_string_CF .= $DIVIDERS{$display_divided_LTR_array[$i+1]};
				$display_string_NF .= $DIVIDERS{$display_divided_LTR_array[$i+1]};
				$i++;
			} else {
				$i=50;
			}
		} elsif ($display_divided_LTR_array[$i] eq "time") {
			$display_string_CF .= getTimeNF();
			unless ($display_divided_LTR_array[$i+1] eq "tail"
				or $display_divided_LTR_array[$i+1] eq ""
					or !exists($display_divided_LTR_array[$i+1])) {
				$display_string_CF .= $COLORS{"DIVIDER"};
			...
			print ("in the volume if \n");
			$display_string_CF .= getVolumeNF();
			$display_string_NF .= getVolumeNF();
			print ($display_string_CF);
			unless ($display_divided_LTR_array[$i+1] eq "tail"
				or $display_divided_LTR_array[$i+1] eq "" 
					or !exists($display_divided_LTR_array[$i+1])) {
				$display_string_CF .= $COLORS{"DIVIDER"};
...
sub printApplets {
	print ( getLeadingSpaces () );
	print ( getDisplayStrings() );
	print ("\n");
}
